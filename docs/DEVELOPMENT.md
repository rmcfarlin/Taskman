...
GitHub Actions runs the test suite on macOS and Ubuntu with Python 3.10 and 3.13, and on Windows with Python 3.13, then builds and smoke-tests each native download on 3.13. A `v*` tag additionally creates a GitHub release with the passing assets. Version tags must match `tui.__version__`. There is no automatic PyPI publication.
